...
/**
  ...
  * \authors Simon BELIER Alexis COMSIS Maxime GUESDON Louis FRANCOIS
*/


void placement_on_board(board plateau, int* new_pr, int* new_pc, tetromino selected_tetromino){
//on demande les coordonnées où placer le tetromino
  ...
  if ( !check_place_tetromino(plateau, *new_pr, *new_pc, selected_tetromino)){
    display_message("Impossible de placer le tetromino ici\n");
...
int main(int argc, char* argv[]){
  ...
  for(;;){
    ...
    //prendre un tetromino dans le sac et le placer
      ...
      //on supprime le tetromino selectionné du sac 
      ...
      if ( !check_place_tetromino(plateau, *new_pr, *new_pc, selected_tetromino)){
        display_message("Impossible de placer le tetromino ici\n");
        add_tetromino_to_bag(plateau, selected_tetromino);
      }
      //si oui : on place le tetromino sur la board
      else{
        place_tetromino(plateau, *new_pr, *new_pc,selected_tetromino);
    ...
    //gestion actions supplémentaires du jeu avancé
    /*
     if (is_advanced){
